...
공개 저장소에는 보고서 본문과 핵심 집계 결과만 포함한다. 보고서 안의 일부 로컬 상세 산출물 링크는 embedding cache, provider 원응답, 전체 payload·근거 package, pair score와 반복 실행 파일을 공개 대상에서 제외했기 때문에 열리지 않을 수 있다.
