...
보고·조회 순서는 **1. JSON↔JSON → 2. OCR↔자체 JSON → 3. OCR↔OCR 보조 진단**입니다. 실행 순서를 강제하는 번호가 아니며 비교는 독립 실행합니다. 문서별 결과는 run 작업 폴더의 `validation/validation_summary.json`에 저장합니다. 관리자 화면을 새로 구현한 것은 아니며, 동일 결과를 터미널에서 조회할 수 있습니다.

```bash
conda run -n skn25 pickcardu-indexer review explain --run-id <run-id> --document-id <issuer/card-id>
```
...
JSON 배열 순서가 달라도 같은 관계를 대응시킵니다. 남은 항목 중 대상·동작의 양방향 유일 후보는 `paired_for_review`로 나란히 보여줄 뿐, 같은 혜택으로 승인하지 않습니다. 실적 구간 등이 겹치는 후보는 `ambiguous`, 대응을 찾지 못하면 `unmatched`입니다. 원문에서 연속 목록으로 확인한 줄 시작 표식만 비교에서 제외하며 수치·조건·한도·예외의 역할은 보존합니다. 원문 및 저장 JSON을 수정하지 않습니다. `1번 pass`도 자체 OCR 근거가 확인되지 않으면 PDF 최종 통과를 뜻하지 않습니다.

검증 계약 v16은 비교·canonical 키에 역할별 값 묶음을 사용합니다. 핵심 혜택은 `target/action/value/unit`, 종속 묶음은 `condition/cap/frequency/period/exceptions`이며 같은 fact에 연결된 상태로 비교합니다. 제목에서는 같은 fact의 핵심 필드에 중복 보존된 내용만 제외하고 남은 고유 문구는 유지합니다. OCR 검증에서는 생략한 제목 수치·대상·동작이 같은 원문 위치의 실제 역할 필드에 보존됐는지도 확인합니다.

`10영업일 이내에/이내`처럼 완전히 식별된 기간 표현과 완전한 전월 실적·건당 금액 술어를 정규화합니다. 명시적 AND로 연결된 인식 가능한 술어만 순서와 무관하게 비교하며, OR·조건과 한도 이동·부정·제외·미지원 문구를 버리지 않습니다. 일반적인 한국어 의미 비교기를 구현한 것은 아닙니다. 묶음 값 비교(`value_bundle`)는 원문 관계 검사와 독립적인 진단이며, 값이 같아도 원격 표·주석의 적용 범위가 불명확하면 최종 검토 필요입니다. 원본 v6 JSON·프롬프트·OCR은 변경하거나 재호출하지 않습니다.
...
`source-manifest.json` is `{"documents":[{"document_id":"issuer/card","source_pdf":"relative-or-absolute.pdf"}]}`. 각 live lane은 `raw_response.*.json`, `pages.json`, `ocr.txt`, `structure_raw_response.*.json`, `structured.json`, `normalized.json`을 별도로 보존합니다. 다페이지 Upstage element는 명시적 page 번호와 모든 페이지의 text를 요구합니다. 로컬 fixture 모드는 provider 디렉터리의 `<issuer__card>.json`을 가져오고 동일한 page/text/normalized 검토 view를 working 디렉터리에 만듭니다. 구조화 입력의 비어 있지 않은 OCR 줄에는 `P0001-L0001` 형식의 ID가 붙으며, identity·fact 근거는 원문을 다시 쓰지 않고 이 ID를 참조합니다. 같은 혜택 블록의 제목·값·조건·한도·예외는 하나의 fact와 여러 `evidence.line_ids`로 묶습니다. 전체 줄 분류를 모델이 다시 출력하지 않고, fact·identity에서 참조하지 않은 줄은 코드가 자동으로 계산합니다. 그중 혜택 키워드가 있는 줄만 `ignored_risky_lines`의 사유를 검사합니다. 두 lane은 한쪽 실패와 관계없이 각각 검증하고, JSON 차이는 항상 진단 artifact로 남기되 자동 canonical 승인은 두 lane이 모두 자체 OCR grounding을 통과한 경우에만 허용합니다.

Provider JSON also requires `provider`, `source_pdf_sha256`, `provenance={endpoint,model,config_hash}`, and `identity={issuer_name,card_name,issuer_evidence,card_evidence}`; new structured identity evidence uses own-lane `line_ids`, while validation still reads legacy page/quote artifacts. A review resolution requires both `selected_provider` and `selected_identity_provider`, a canonical `identity` with exact per-lane identity evidence and `supports_selected`, and the complete selected-provider `canonical` relation set. `rejected_relations` contains exactly the non-selected lane's tuple-set difference as `{provider,tuple,reason}` entries. Missing, extra, duplicate, mixed-lane, or unsupported facts and identities fail closed.
